...
func main() {
	db, err := bbolt.Open("people.db", 0666, nil)
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
...
func addInitialData(db *bbolt.DB) error {
	err := db.Update(func(tx *bbolt.Tx) error {
		personBucket, err := tx.CreateBucketIfNotExists([]byte("people"))
		if err != nil {
			return err
		}
		nameIndexBucket, err := tx.CreateBucketIfNotExists([]byte("name_index"))
		if err != nil {
			return err
		}
		birthIndexBucket, err := tx.CreateBucketIfNotExists([]byte("birth_index"))
		if err != nil {
			return err
		}
...
func searchByBirthRange(startDateStr, endDateStr string, db *bbolt.DB) {
	...
	err = db.View(func(tx *bbolt.Tx) error {
		...
		for k, v := cursor.Seek(startKey); k != nil && string(k) <= string(endKey); k, v = cursor.Next() {
			...
			birthDate, err := time.Parse("2006-01-02", p.Birth)
			if err != nil {
				return err
			}
